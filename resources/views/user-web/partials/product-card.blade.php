...
<article
    ...
    <a href="{{ $ui->productUrl($product) }}" class="relative aspect-[4/5] overflow-hidden">
        ...
        @endif
        <span
            class="absolute left-4 top-4 rounded-full bg-primary px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-on-primary shadow-lg">
            {{ $product->region?->name ?? ($product->category?->name ?? 'Đặc sản') }}
        </span>
    </a>
    <div class="flex flex-1 flex-col space-y-3 p-6">
        <div class="flex items-start justify-between gap-3">
            <a href="/{{ $ui->productUrl($product) }}"
                class="font-headline text-lg font-semibold leading-tight transition-colors group-hover:text-primary">
                {{ $product->name }}
            </a>
            <div class="flex shrink-0 items-center text-tertiary">
                @include('user-web.partials.icon', [
                    'name' => 'star',
                    'class' => 'text-[16px]',
                    'fill' => true,
                ])
        ...
        <p class="line-clamp-2 text-sm leading-relaxed text-on-surface-variant">
        ...
        <div class="mt-auto flex items-center justify-between gap-3 pt-4">
            ...
            <form action="{{ route('user-web.cart.items.store') }}" method="POST">
                ...
                <button
                    class="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-on-primary transition-all active:scale-90 disabled:cursor-not-allowed disabled:bg-surface-container-high disabled:text-on-surface-variant"
                    type="submit" @disabled(!$isAvailable) aria-label="Thêm vào giỏ {{ $product->name }}">
